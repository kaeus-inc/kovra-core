---
title: Flujos
description: Los distintos flujos que admite kovra — inyección en proceso, uso por agente sobre MCP, revelación humana, compartición sellada, intercambio por USB, ssh-agent y la interfaz web — cada uno con un diagrama.
---

kovra mueve un secreto de algunas maneras distintas. Cada **flujo** a continuación corresponde a un escenario, con un diagrama que muestra la ruta que toma el valor — y, igual de importante, a dónde **no** se le permite ir. Todos pasan por la misma verificación subyacente; consulta [el proceso de decisión](/es/security/decision/) para saber cómo funciona esa verificación.
...
El flujo cotidiano. Conectas los nombres de variables a coordenadas en `.env.refs`, ejecutas tu herramienta *a través* de kovra, y los valores resueltos van **directamente al proceso** — nunca al disco, a argv ni a tu pantalla. El valor se usa, no se ve.

<figure class="flow-diagram">
<svg viewBox="0 0 680 150" role="img" aria-label="Flujo de inyección en proceso">
...
El flujo principal. Un agente de IA trabaja bajo un [alcance](/es/concepts/agent-scope/): lee **metadatos** y puede hacer que kovra **inyecte** secretos en los comandos que ejecuta, para que funcionen — pero el **texto plano sensible nunca entra al contexto del modelo**, el único lugar donde un ataque de inyección de prompts podría exfiltrarlo.

<figure class="flow-diagram">
<svg viewBox="0 0 680 200" role="img" aria-label="Flujo de agente sobre MCP">
...
## Revelar un secreto a un humano

A veces *tú* necesitas el valor. Una revelación se juzga por sensibilidad: un secreto ordinario se muestra; uno `high` se muestra solo después de un <span class="bioprove">bioProve</span>; el más protegido (`inject-only`) **nunca se muestra** — solo puede inyectarse. Un agente nunca puede activar esto por ti.

<figure class="flow-diagram">
<svg viewBox="0 0 680 216" role="img" aria-label="Flujo de revelación humana">
...
## Compartir un conjunto de secretos (paquete sellado)

Para entregar secretos a otra persona, kovra **sella** un conjunto no productivo a la clave pública del destinatario. Solo ese destinatario puede abrirlo — con **su propia identidad** — y un **token de acceso** de un solo uso viaja por un canal distinto para autorizar las entradas más sensibles. Los secretos de producción son rechazados directamente.

<figure class="flow-diagram">
<svg viewBox="0 0 680 190" role="img" aria-label="Flujo de compartición sellada">
...
## Inicializar una máquina nueva (intercambio USB)

La transferencia sin conexión completa hacia una máquina que todavía no tiene kovra. El USB hace **dos viajes** — hacia afuera con las herramientas, de regreso con la identidad del destino, hacia afuera de nuevo con el paquete sellado — y el **token de acceso viaja por separado**. Cada paso destructivo o sensible está bloqueado por un <span class="bioprove">bioProve</span>, y la producción está excluida.

<figure class="flow-diagram">
<svg viewBox="0 0 680 300" role="img" aria-label="Intercambio sin conexión USB — diagrama de colaboración">
  <rect class="box" x="40" y="18" width="200" height="44" rx="8"/>
  <text class="t" x="140" y="40" text-anchor="middle">Origen</text>
  <text class="s" x="140" y="55" text-anchor="middle">tiene kovra + vault</text>
  ...
  <text class="t" x="540" y="40" text-anchor="middle">Destino</text>
  <text class="s" x="540" y="55" text-anchor="middle">sin kovra aún</text>
  ...
  <text class="s" x="340" y="290" text-anchor="middle">Cada paso destructivo está bloqueado por un bioProve · la producción está excluida.</text>
</svg>
</figure>

## Autenticarse con una clave custodiada (ssh-agent)

kovra puede actuar como un ssh-agent gobernado: un cliente SSH o git envía un desafío, kovra lo firma **en memoria** con una clave custodiada y devuelve la firma. La **clave privada nunca sale de kovra** y nunca toca el disco; las claves `high`/`prod` requieren una confirmación en **cada** firma.

<figure class="flow-diagram">
<svg viewBox="0 0 680 168" role="img" aria-label="Flujo del ssh-agent gobernado">
  ...
  <text class="s" x="340" y="148" text-anchor="middle">Firmado en memoria — la clave privada nunca sale de kovra; high/prod confirman cada vez.</text>
</svg>
</figure>

## Administrar desde el navegador (interfaz web)

Una interfaz de administración bajo demanda, lanzada tras una confirmación y vinculada **solo a loopback**. Muestra los valores ordinarios pero **nunca renderiza** el texto plano de secretos `high` o `inject-only` — estos aparecen enmascarados, con una huella digital, y solo se revelan a través de la CLI.

<figure class="flow-diagram">
<svg viewBox="0 0 680 196" role="img" aria-label="Flujo de la interfaz web">
  ...
  <text class="s" x="340" y="186" text-anchor="middle">Solo loopback — nunca expuesto a la red.</text>
</svg>
</figure>
